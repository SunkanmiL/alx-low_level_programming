 ...
 *         Otherwise - a pointer to the newly created sorted hash table (heap)
 */
shash_table_t *shash_table_create(unsigned long int size)
{
	shash_table_t *ht;
 ...
 * @ht: A pointer to the sorted hash table to set in
 * @key: The key to set in hash table - cannot be an empty string
 ...
 * Return: The value associated to key, or NULL if key couldn't not found
 */
char *shash_table_get(const shash_table_t *ht, const char *key)
{
	shash_node_t *node;
